...
{$id_lang = Context::getContext()->language->id}
	...
	<ul class="menu-content">
		{foreach from=$menus item=menu name=menus}
			...
			{else}
				<li class="level-1 {if $menu.type_icon Or $menu.icon }laberIcon{/if} {$menu.class|escape:'html':'UTF-8'}{if count($menu.sub_menu) > 0} parent{/if}">
					
					<a href="{$menu.link|escape:'html':'UTF-8'}">
					{if $menu.type_icon == 0 && $menu.icon != ''}
						<span class="img-icon"><img  src="{$icon_path|escape:'html':'UTF-8'}{$menu.icon|escape:'html':'UTF-8'}" alt=""/></span>
