			...
			<div class="laberProdCategory labProdCat-{$cat_info.id_cat|intval}">
				<div class="title_block">
					...
					<h3>
						<a href="{$link->getCategoryLink($cat_info.id_cat, $cat_info.link_rewrite)|escape:'html':'UTF-8'}" title="{$cat_info.cat_name|escape:'html':'UTF-8'}">
							{$cat_info.cat_name|escape}
						</a>
					</h3>
				  
				
				{if $show_sub}
					<span class="sub_category sub_category-{$cat_info.id_cat|intval} hidden-md-up"><i class="material-icons d-inline">&#xE5D2;</i></span>
					<ul class="tab_cates">
						{foreach from = $cat_info.sub_cat item=sub_cat name=sub_cat_info}
							{if $smarty.foreach.sub_cat_info.iteration <=3}
				...
				<div class="product_list labContent">
						...
						<div class="owl-buttons">
							<div class="owl-prev prev-{$id}{$cat_info.id_cat|intval}"><i class="fa fa-angle-left icon"></i></div>
							<div class="owl-next next-{$id}{$cat_info.id_cat|intval}"><i class="fa fa-angle-right icon"></i></div>
						</div>
						{/if}
						{*{if count($cat_info)>0}
						<div class="manu-list">
